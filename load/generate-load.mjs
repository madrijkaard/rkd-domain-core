...
for (const domain of files) {
  for (const field of domain.fields) {
    if (field.type === 'OPTION' && field.choices?.length) {
      options.set(`${domain.code}__${field.code.toUpperCase()}`, {
...
const lines = [
  ...
  '', 'BEGIN;', '', '-- Hard reset: this file is intended for test data only.',
  'DROP TABLE IF EXISTS domain_relation CASCADE;',
  ...
  '  attribute_id BIGINT NOT NULL UNIQUE REFERENCES attribute(id)',
  ');', '',
...
for (const domain of files) {
  ...
  for (const field of domain.fields) {
    ...
    lines.push('INSERT INTO attribute (code, description, type, mandatory, domain_id, status, created_at, updated_at)',
      `SELECT ${sql(field.code)}, ${sql(field.description)}, ${sql(field.type)}, ${String(field.mandatory).toUpperCase()}, d.id, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP`,
...
lines.push('-- Options');
for (const [code, optionDefinition] of options) {
  const { choices, attributeCode } = optionDefinition;
  const value = JSON.stringify({ options: choices.map(([choice, label]) => ({ value: choice, label })) });
  const [domainCode] = code.split('__');
  lines.push('INSERT INTO "option" (code, description, "values", attribute_id, status, created_at, updated_at)',
    `SELECT ${sql(code)}, ${sql(`Options for ${code}`)}, ${sql(value)}::jsonb, a.id, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP`,
    `FROM attribute a JOIN domain d ON d.id = a.domain_id WHERE d.code = ${sql(domainCode)} AND a.code = ${sql(attributeCode)}`,
    `  AND NOT EXISTS (SELECT 1 FROM "option" o WHERE o.code = ${sql(code)});`,
    `UPDATE "option" o SET attribute_id = a.id`,
    `FROM attribute a JOIN domain d ON d.id = a.domain_id`,
    `WHERE o.code = ${sql(code)} AND d.code = ${sql(domainCode)} AND a.code = ${sql(attributeCode)}`,
    `  AND o.attribute_id IS NULL;`, '');
}
lines.push(
  'DO $$',
  'BEGIN',
  "  IF (SELECT COUNT(*) FROM attribute WHERE type = 'OPTION') <> (SELECT COUNT(*) FROM \"option\") THEN",
  '    RAISE EXCEPTION \'OPTION attributes and option rows are not fully mapped.\';',
  '  END IF;',
  'END $$;', '',
  'COMMIT;', '');
